...
ดูศัพท์โดเมนที่ [CONTEXT.md](../CONTEXT.md) · วงจรชีวิตออเดอร์ที่ [order-lifecycle.md](./order-lifecycle.md) · เหตุผลเชิงตัดสินใจที่ [adr/](./adr/)
...
- nit: ข้อความ 403 ของ `requireMerchantOf` เดิมเขียน "แก้ได้เฉพาะเมนูร้านของตัวเอง" แต่ใช้กับ transition ออเดอร์ด้วย (กำกวม) → **แก้เป็น "จัดการได้เฉพาะร้านของตัวเอง"** (neutral ถูกทั้ง menu CRUD + order transition)
...
**Critique fixes**: (1) **stats hero ให้จริง** — เดิมฮาร์ดโค้ด "เปิดอยู่ 23 ร้าน / ไรเดอร์ 12 คน" → `inZoneCount` (ร้าน serviceable จริงจากที่อยู่ที่ปักหมุด, reactive) + ตัดเลขไรเดอร์ปลอมทิ้ง. (2) **glow มีวินัย** — ลบ glow จาก `.btn--mango` (เดิมเรืองเกลื่อน Admin/Login); สงวน glow ให้ "ปุ่มร้อน = action หลักต่อหน้า" เท่านั้น (`.btn--chili` สั่งเลย/เพิ่มลงตะกร้า + cartbar + OrderTracker สถานะ live). (3) **hero กระชับ (distill)** — เก็บป้ายไฟ (signature) ไว้ แต่ลด spacing รอบๆ (topbar/eyebrow/open-tag/riders/search margins) ~56px บนมือถือ → ช่องค้นหา+ร้านขึ้นเหนือ fold เร็วขึ้น (เดสก์ท็อปคง padding-top โปร่งจาก responsive block). typecheck + UI 76/76 ผ่าน.

**Loading state** (`hydrating` flag): แอป seed-first + optimistic → เนื้อหาไม่เคยว่าง เลยใช้ **background-refresh loading bar** แทน content skeleton (ซึ่งจะไปแทนเนื้อหาที่โชว์อยู่แล้ว). เพิ่ม `hydrating` ใน `StoreValue` (React state ใน StoreProvider — set true ตอน `rehydrate` → false ใน finally; ไม่แตะ reducer/State literal → เทสต์ไม่กระทบ) + `<LoadingBar/>` ใน App.tsx (แถบบางบนสุด indeterminate, **delay 250ms กันกระพริบ** ตอนโหลดเร็ว, `.loadbar` ใน App.css GPU transform, reduced-motion → static). โชว์ตอน hydrate ดึงข้อมูลสดจาก backend (เห็นผลจริงบนเน็ตช้า). UI 76/76.
...
## ภาพรวมฟีเจอร์ (โดเมน + client — ฐาน V1)

> รายการด้านล่างคือฟีเจอร์ **ฐาน V1 (โดเมน + client)**. ส่วน **backend/auth/API cutover/QA/design polish อยู่ในหัวข้อด้านบน** — ปัจจุบันเป็น **fullstack จริง** (React ↔ Fastify ↔ Postgres + Lucia + pg-boss), UI tests **76/76**, domain **109/109**, QA issues #1–#6 ปิดครบ.
...
- [x] **Backend จริง — ทำแล้ว**: Fastify + Postgres(Drizzle) + **Lucia auth (session cookie)** + pg-boss(cron); store→API cutover เต็ม — ทุก transition/ตัวตน(4 ฝั่งจาก session)/การเงิน persist ผ่าน DB, auth ตามบทบาท, resilient ต่อ DB drop (ดูหัวข้อด้านบน)
- [x] **หน้าแรก/UX wired**: 📍 เปลี่ยนที่อยู่ + แผนที่ Leaflet, `/all`, per-route `document.title`, **responsive ทั้งแอป** (tablet/desktop), feedback micro-interactions + loading bar
- [x] **QA hardening**: 6 issues (#1–#6) แก้+ปิดครบ — hardcoded lists / settlement-on-delivery / auth gate complete-cancel / validate POST orders / pg-boss resilience / per-route title
- [ ] **แก้เมนูระดับ option group/extras** ฝั่ง Merchant + ลิสต์หลายออเดอร์พร้อมกันในคอนโซล (ตอนนี้ CRUD ระดับเมนู: ชื่อ/ราคา/คำอธิบาย)
- [ ] **รูปอาหารจริงแทน emoji** + master-detail consoles + per-surface skeleton (design critique — asset/component-dependent)
- [ ] **deploy/hosting จริง**: cron ผูกเซิร์ฟเวอร์ที่รันแม้ไม่เปิดหน้า, โฮสต์ DB/แอปจริง (ตอนนี้รัน local + Docker Postgres :5433)
